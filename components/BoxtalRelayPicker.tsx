...
import type { BoxtalParcelShop, DayHours } from "@/app/api/boxtal-parcelshops/route";

export type { BoxtalParcelShop };

type Props = {
  postalCode: string;
  onSelect: (point: BoxtalParcelShop) => void;
  selectedPoint: BoxtalParcelShop | null;
...
const NETWORK_LABELS: Record<string, { label: string; color: string }> = {
  MONR: { label: "Mondial Relay", color: "bg-yellow-500/20 text-yellow-400" },
  CHRP: { label: "Chronopost",    color: "bg-orange-500/20 text-orange-400" },
};
...
function isOpenNow(hours: BoxtalParcelShop["hours"]): boolean {
  const now = new Date();
...
export default function BoxtalRelayPicker({ postalCode, onSelect, selectedPoint, t }: Props) {
  const [points, setPoints]             = useState<BoxtalParcelShop[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/boxtal-parcelshops", {
      ...
      body: JSON.stringify({ postalCode, networks: "MONR,CHRP" }),
    })
      .then(r => r.json())
  ...
  if (!points.length) {
    return (
      <p className="text-center py-4 text-gray-500 text-xs">
        {t({ fr: "Aucun point relais trouvé pour ce code postal.", en: "No relay point found for this postal code." })}
      </p>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                >
                  <Popup>
                    <div className="text-xs">
                      ...
                      <p className="text-gray-500 text-[11px]">{NETWORK_LABELS[sp.network]?.label ?? sp.network}</p>
        ...
        <div className="flex flex-col gap-1.5 max-h-[280px] overflow-y-auto pr-0.5 custom-scrollbar">
          {points.map((sp, i) => {
            ...
            const net        = NETWORK_LABELS[sp.network];
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {net && (
                          <span className={`flex-shrink-0 text-[9px] px-1.5 py-0.5 rounded font-medium ${net.color}`}>
                            {net.label}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded ${
                          ...
                          {open ? t({ fr: "Ouvert", en: "Open" }) : t({ fr: "Fermé", en: "Closed" })}
                        </span>
                      </div>
                      {sp.distance > 0 && (
                  ...
                  <div className={`border border-t-0 rounded-b-lg px-3 py-2 ${
                    ...
                    <div className="grid grid-cols-1 gap-0.5">
                      {DAYS_FR.map((day, di) => {
                        ...
                        return (
                          <div key={day} className={`flex justify-between text-[10px] py-0.5 px-1 rounded ${today ? "bg-white/[0.04]" : ""}`}>
        ...
        <div className="flex items-start gap-2 bg-green-500/10 border border-green-500/25 rounded-lg px-3 py-2.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {NETWORK_LABELS[selectedPoint.network] && (
                <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${NETWORK_LABELS[selectedPoint.network].color}`}>
                  {NETWORK_LABELS[selectedPoint.network].label}
                </span>
              )}
            </div>
